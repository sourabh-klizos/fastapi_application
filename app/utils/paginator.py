...
async def check_has_pre_next(collection:Collection,query:Dict[str ,Any], content_per_page:int , current_page:int )-> Dict[str,bool]:
...
async def paginate_query(
    ...
    has_next_or_prev = await check_has_pre_next(query=query, collection=collection,content_per_page=per_page,current_page= page )
    response = {
        **has_next_or_prev,
        "data" :data
