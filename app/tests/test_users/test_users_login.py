...
@pytest.mark.asyncio
async def test_limit_no_of_records(
    client, get_test_db, get_admin_user_token, create_bulk_test_users
):
...
async def test_access_other_user(
    ...
    other_user_id = await user_collection.find_one(
        {"email": other_user_detail["email"]}, {"_id": 1}
    )
    str_other_user_id = str(other_user_id["_id"])
    response = await client.get(
        f"/api/v1/auth/users/{str_other_user_id}", headers=headers
    )
...
@pytest.mark.asyncio
async def test_update_user_detail_success(
    client, get_test_db, get_admin_user_token, test_user
):
...
@pytest.mark.asyncio
async def test_update_self_user_detail(
    client, get_test_db, get_current_user_token, test_user
):
...
@pytest.mark.asyncio
async def test_update_other_user_detail_without_admin_previllage(
    client, get_test_db, get_current_user_token, test_user
):
    token = get_current_user_token["access_token"]
    ...
    response = await client.put(
        f"/api/v1/auth/users/{str_other_user_id}", headers=headers, json=data
    )
